...
def total_value_size(packing, values, sizes, max_size):
...
def solve(n_items, rnd, values, sizes, max_size, max_iter, start_temperature, alpha):
    ...
    (curr_value, curr_size) = total_value_size(curr_packing, values, sizes, max_size)
    iteration = 0
    interval = int(max_iter / 10)
    while iteration < max_iter:
        adj_packing = adjacent(curr_packing, rnd)
        (adj_v, _) = total_value_size(adj_packing, values, sizes, max_size)

        if adj_v > curr_value:  # better so accept adjacent
        ...
        #     print("iter = %6d : curr value = %7.0f : curr temp = %10.2f " % (iteration, curr_value, curr_temperature))
...
def main():
    ...
    # print("\nStarting solve() ")
    packing = solve(10, rnd, values, sizes, max_size, max_iter, start_temperature, alpha)
    # print("Finished solve() ")

    print("\nBest packing found: ")
    print(packing)
    (v, s) = total_value_size(packing, values, sizes, max_size)
    print("Total value of packing = %0.1f " % v)
